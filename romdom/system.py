"""
RomDom System classes - class used to iterate over all of the roms for a given system
"""
...
class BaseSystem(object):
    ...
    def __repr__(self):
        return '<%s: %s (%s)>' % (self.__cls__.__name__, self.system_name, self.url)
    ...
    def get_rom_list(self, page):
        ...
        matcher = re.compile(self.rom_regex % self.url.rsplit('/', 2)[-2])
...
class CoolRomSystem(object):
    """Class for browsing roms of a particular system"""
    rom_regex = r'<a href="(/roms/%s/\d*?/[^<>]*?.php)">\s*(?!<img)([^<>]*?)</a>'
